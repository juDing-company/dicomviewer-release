...
function ImageTransfer() {
    ...
    var PngDlSuccess = function (buffer, item) {
        ...
        if (item instanceof Object) {
            var id = item.id;/*DicomId*/
            var dcmWidth = item.dcmwidth;/*Dicom宽度*/
            var dcmHeight = item.dcmheight;/*Dicom高度*/

            var undecode;
            var grayBuffer;
            var bseries = item.bseries;
            if (bseries) {/*序列图像*/
                if (item.bsave) {/*需要解码保存到回去使用*/
                    ...
                    /*传真实数据回去*/
                    self.postMessage({
                        id: id, buffer: grayBuffer.buffer, width: m_Png.width,
                        height: m_Png.height, bseries: item.bseries, brgb: item.brgb
                    }, [grayBuffer.buffer]);

                } else {
                    //console.log('未保存');
                    self.postMessage({
                        id: id, width: m_Png.width,
                        height: m_Png.height, bseries: item.bseries, brgb: item.brgb
                    });
                }
                ...
                // console.log(index);
                //  grayBuffer = new Uint8Array(buffer);
                if (index == 0) {
                    var record = {};
                    record.id = id;
                    record.complete = 1;
                    ...
                    if (record != undefined || record != null) {
                        ...
                        if (record.complete > (record.total - 1)) {/*发送合并完成的图像*/
                            self.postMessage({
                                id: id, buffer: record.mergebuffer.buffer, width: record.width,
                                height: record.height, bseries: item.bseries, brgb: item.brgb/*, progress: parseInt(record.complete / record.total * 100)*/
                            }, [record.mergebuffer.buffer]);
                            ...
                            //self.postMessage({
                            //    id: id, buffer: null, width: m_Png.width,
                            //    height: m_Png.height, bseries: item.bseries, brgb: item.brgb
